...
export const useSiteData = () => {
    ...
    const [siteData, setSiteData] = useState(() => {
        ...
        if (saved) {
            ...
            const oldDefaultNames = ['Sabrina De Martini', 'beca barreto'];
            const oldDefaultHandles = ['@sabrinademartini', '@becabarreto'];

            if (oldDefaultNames.includes(parsed.name)) parsed.name = DEFAULT_DATA.name;
            if (oldDefaultHandles.includes(parsed.handle)) parsed.handle = DEFAULT_DATA.handle;
